...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create my-epic-app-5a5e
  fly apps create my-epic-app-5a5e-staging
  ```
...
- Add a `SESSION_SECRET`, `ENCRYPTION_SECRET`, and `INTERNAL_COMMAND_TOKEN` to
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) ENCRYPTION_SECRET=$(openssl rand -hex 32) INTERNAL_COMMAND_TOKEN=$(openssl rand -hex 32) --app my-epic-app-5a5e
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) ENCRYPTION_SECRET=$(openssl rand -hex 32) INTERNAL_COMMAND_TOKEN=$(openssl rand -hex 32) --app my-epic-app-5a5e-staging
  ```
...
- **Create an account on Mailgun.** (Can be deferred to later)
  ...
  ```sh
  fly secrets set MAILGUN_DOMAIN="mg.example.com" MAILGUN_SENDING_KEY="some-api-token-with-dashes" --app my-epic-app-5a5e
  fly secrets set MAILGUN_DOMAIN="mg.example.com" MAILGUN_SENDING_KEY="some-api-token-with-dashes" --app my-epic-app-5a5e-staging
  ```
...
- Create a persistent volume for the sqlite database for both your staging and
  ...
  ```sh
  fly volumes create data --size 1 --app my-epic-app-5a5e
  fly volumes create data --size 1 --app my-epic-app-5a5e-staging
  ```
